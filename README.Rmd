...
My cleaned data has 'r ncol(cleaned_data)' columns and 'r nrow(cleaned_data' rows. My tidy data, which separates the 6, 18, and 24 hour time pints into separate columns, set has 'r ncol(tidy_data)' columns and 'r nrow(tidy_data)' rows. The mean of the hpi.24 columns is 'r mean(tidy_data, hpi.24)' and the standard deviation is 'r sd(tidy_data, hpi.24)'.
...
tidy_data %>% mutate(exp.diff = hpi.24 - hpi.6) %>% 
 ...
 #this table is a subset of cleaned_data tbl showing only the gene names from the "top 3 genes" list, found above; I know this isn't quite right because I have 144 rows and should only have 36. Do I need to group things differently?
cleaned_data %>% semi_join(top3genes,by = "gene.name") -> cleaned_top3genes

#make table showing average expression across timepoints
cleaned_top3genes %>% group_by(gene.name, time.point) %>% mutate("avg.expr" = mean(value)) -> cleaned_top3genes_expr
cleaned_top3genes_expr

#plot the points
ggplot(cleaned_top3genes_expr, aes(x = time.point, y = value)) + geom_point(aes(color = gene.name, shape = rep)) + geom_smooth(method = 'lm')
